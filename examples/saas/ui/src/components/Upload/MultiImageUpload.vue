...
const props = defineProps({
  ...
  /**
   * 删除前回调函数，返回Promise，用于删除数据库关联等操作
   * 参数：imageUrl - 图片URL
   * 返回：Promise<boolean> - true表示允许删除，false表示取消删除
   */
  onBeforeRemove: {
    type: Function as PropType<(imageUrl: string) => Promise<boolean> | boolean>,
    default: null,
  },
});
...
watch(
  ...
  (newValue) => {
    ...
    // 保存现有的 fileList，以便保留已上传文件的 uid
    const existingFiles = new Map<string, UploadUserFile>();
    fileList.value.forEach((file) => {
      if (file.url) {
        existingFiles.set(file.url, file);
      }
    });

    // 根据 modelValue 更新 fileList
    fileList.value = newValue.map((url) => {
      // 检查是否已存在于 fileList 中，保留原有的文件对象（包括 uid）
      const existing = existingFiles.get(url);
      if (existing) {
        return existing;
      }
      // 创建新的文件项
...
async function handleRemove(imageUrl: string) {
  ...
  if (props.onBeforeRemove) {
    try {
      const canRemove = await props.onBeforeRemove(imageUrl);
      if (canRemove === false) {
        // 取消删除
        return;
      }
    } catch (error) {
      console.error("删除前回调失败:", error);
...
// onMounted 已不需要，因为 watch 会在初始化时执行
</script>
